...
/** Grid-bucketed spatial index so the canvas only asks "what's near the
 * viewport" instead of scanning every entity every frame — the load-bearing
 * piece that keeps a campaign with thousands of entities from ever hitting
 * a real O(n) render cost during pan/zoom. */
