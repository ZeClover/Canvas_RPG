# RPG World Canvas — v0.9.0 (Fase 7 + extras + Fase 8)
...
- 30 tipos de card (29 tipos de conteúdo + grupo), cada um com ícone, cor padrão e tamanho padrão configuráveis;
...
Todas as 7 fases do plano original, os nove extras pós-Fase 7 e os cinco módulos da Fase 8 foram implementados. O que resta é puramente deferido por escopo (não por dificuldade): auto-layout automático do Canvas, temas além do escuro padrão.
...
```bash
npm test           # 194 testes automatizados (Vitest) — inclui IndexedDB real via fake-indexeddb
...
- Criar um elemento por duplo clique sempre cria um NPC por padrão (não há como perguntar "qual tipo?" num duplo clique); use o seletor de tipo na barra de ferramentas do canvas para os outros 28 tipos.
