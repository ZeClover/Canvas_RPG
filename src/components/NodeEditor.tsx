...
export function NodeEditor({ initialValue, bounds, onCommit, onCancel }: NodeEditorProps) {
  const [value, setValue] = useState(initialValue === "Nova caixa" ? "" : initialValue);
    ...
    <textarea
      ...
      onKeyDown={(event) => {
        ...
        if (event.key === "Enter" && !event.shiftKey) {
          event.preventDefault();
          onCommit(value.trim() || "Sem título");
        }
      }}
    />
  );
